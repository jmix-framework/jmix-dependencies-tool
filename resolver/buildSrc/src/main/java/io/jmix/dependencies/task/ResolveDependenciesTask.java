...
public class ResolveDependenciesTask extends DefaultTask {
    ...
    private void resolveSourcesArtifacts() {
        Project project = getProject();
        project.getConfigurations().stream()
                .filter(Configuration::isCanBeResolved)
                .forEach(configuration -> {
                    configuration.getResolvedConfiguration().getResolvedArtifacts().forEach(resolvedArtifact -> {
                        project.getDependencies().createArtifactResolutionQuery()
                                .forComponents(resolvedArtifact.getId().getComponentIdentifier())
    ...
    private void addExternalDependencies() {
        ...
        if (externalDependencies != null) {
            logger.info("Explicit dependencies: {}", externalDependencies);
