...
public class JmixGradleClient {
    ...
    /**
     * Baseline JVM args for the resolution build daemon. Gradle's 512 MiB default heap makes
     * {@code vaadinBuildFrontend} thrash the GC and die on a full module set (especially with commercial
     * add-ons); a larger metaspace covers the many modules/plugins on the classpath. Written into the
     * generated {@code gradle.properties}; a caller that supplies its own {@code org.gradle.jvmargs}
     * (e.g. via {@code --gradle-jvmargs}) wins.
     */
    public static final String DEFAULT_GRADLE_JVM_ARGS = "-Xmx2g -XX:MaxMetaspaceSize=512m";
    ...
    private void writeGradleProperties(Map<String, String> properties) {
        ...
        // Raise the daemon heap so vaadinBuildFrontend / a full module set does not OOM on Gradle's default.
        props.putIfAbsent("org.gradle.jvmargs", DEFAULT_GRADLE_JVM_ARGS);
